...
### 四：用@Configuration配置spring并加载spring容器
@Configuration标注在类上，@Configuation等价于spring的xml配置文件中的<Beans></Beans>

#### 步骤1：pring的依赖包
STS已集成spring-context，无需添加依赖关系
...
### 四：将对象注册到Spring IoC 容器中
要把一个对象注册到SpringIoC容器中，一般是用@Bean注解来实现。
...
``` 
<beans>
   <bean id="userBean" class="com.coffee.UserBean" />
</beans>
```
#### 步骤3：加一个体验类
创建的bean对象，通过AnnotationConfigApplicationContext加载进spring ioc 容器中。
``` 
    public static void main(String[] args) {
        ApplicationContext context = new AnnotationConfigApplicationContext(MyConfiguration.class);
...
``` 
MyConfiguration初始化。。。
UserBean{username='liuwei', password='111111'}
Process finished with exit code 0
```
